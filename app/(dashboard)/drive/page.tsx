...
function FileTable({
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Create a folder or upload files to get started</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden">
      {/* Header row */}
      <div className="grid grid-cols-[32px_minmax(200px,2fr)_minmax(140px,1fr)_minmax(140px,1fr)_48px] items-center px-6 h-10 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider border-b border-border bg-muted/30 sticky top-0 z-10">
        <div />
      ...
      <div className="flex justify-center opacity-0 group-hover:opacity-100 transition-opacity">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="w-7 h-7 flex items-center justify-center text-muted-foreground hover:bg-muted rounded transition-colors"
...
function GridView({
  ...
  return (
    <div className="space-y-6 mt-4">
      {[
        { label: "My Folders", items: myFolders },
                ...
                >
                  ...
                  {folder.isOwner && (
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button
                          className="absolute top-1 right-1 w-6 h-6 flex items-center justify-center text-muted-foreground hover:bg-muted rounded opacity-0 group-hover:opacity-100 transition-opacity"
                          onClick={(e) => e.stopPropagation()}
                        >
        ...
        <section className="space-y-3">
          ...
          <div className="rounded border overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {files.map((file, i) => (
                  <tr key={file.id} className={cn("border-t hover:bg-muted/20 transition-colors", i === 0 && "border-t-0")}>
